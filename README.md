...
- Перед установкой зависимостей выполни `nvm use 20` (см. `.nvmrc`).
  Для автоматического переключения можно запустить `sh scripts/setup-node.sh`.
...
nvm use       # выполните эту команду перед установкой зависимостей
              # (скрипт scripts/check-node.js напомнит версию из .nvmrc)
# для удобства запустите скрипт, который проверит nvm и выполнит nvm use
...
> **Важно:** перед `pnpm install` обязательно выполните `nvm use $(cat .nvmrc)`.
> Без этого установка зависимостей может завершиться ошибкой.
> Для удобства запустите `sh scripts/setup-node.sh` — он проверит наличие `nvm`
> и автоматически вызовет `nvm use`.
...
Шрифты и фоновые изображения шаблона Majestic не хранятся в репозитории (~3.8 MB).
